...
// 定义应用目录
define('APP_PATH', __DIR__ . '/app/controller');
...
while (($module_name = readdir($module_dir)) !== false) {
    $module_path = APP_PATH . DIRECTORY_SEPARATOR . $module_name;    //构建子目录路径/app/controller/api

    if (is_dir($module_path)) {
        $module = strtolower($module_name);//api
        $module_child_dir = opendir($module_path);//-app/controller/api
        ...
        while (($module_child_name = readdir($module_child_dir)) !== false) {
            ...
            if (is_dir($module_child_path) && $module_child_name == 'api') {
...
//												$parameter['collectionFormat'] = 'multi'; 
                                    ...
                                    // 生成api访问路径
                                    $paths['/api/' . $controller_name . '/' . $path][$method] = $func;
                                }
                            }
                        }
                    }
