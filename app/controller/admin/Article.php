...
class Article extends Controller
{
     ...
     * 列表
     */
    public function index($status = 99, $keyword = "", $p = 1)
    {
        //通用搜索
        $where = "isdel = 0";
        ...
        //统计总数
        $sql = "SELECT count(*) as c FROM xyq_article where " . $where;
        $total = $this->db->query($sql)->row_array();
        $total = $total["c"];

        //查询记录
        $sql = "SELECT id,title,status,hits,addtime FROM xyq_article where " . $where . " order by id desc limit $start, $size";
        $list = $this->db->query($sql)->result_array();

        //分页展示
        $page = page_format($total, $size, $p, $status . "/" . $keyword);
    ...
    public function add()
    {
        if (IS_POST) {
            ...
            $content = isset($_POST["content"]) ? sql_format($_POST["content"]) : "";
            ...
            if (!$sdesc) $sdesc = strip_tags(substr($content, 200));
            if (!$intro) $intro = strip_tags(substr($content, 200));
